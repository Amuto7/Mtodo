...
export const UserstaskSlice = createSlice({
  ...
  reducers: {
    ...
    deleteTask: (state, { payload }) => {
      const update = state.users.filter((e) => e.id !== payload.id);
      state.users = update;
      const upd = state.completedtasks.filter((e) => e.id !== payload.id);
      state.completedtasks = upd;
      const upda = state.importentTasks.filter((e) => e.id !== payload.id);
      state.importentTasks = upda;
      const up = state.searched.filter((e) => e.id !== payload.id);
      state.searched = up;
      localStorage.setItem("users", JSON.stringify(state.users));
    },
